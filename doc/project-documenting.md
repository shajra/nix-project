...
- [Org2gfm design](#sec-5)
  ...
  - [No evaluation when exporting](#sec-5-2)
...
This document shows a way to document your code with the `org2gfm` script provided by this project, Nix-project. This script converts [Emac's Org-mode](https://www.gnu.org/software/emacs/manual/html_node/emacs/Org-Mode.html) files into [GitHub Flavored Markdown (GFM)](https://github.github.com/gfm/). It can evaluate all source blocks in the Org files first to generate results to include in the documentation or check that source code snippets work as intended.
...
Documenting programming projects often requires embedding code snippets. We often want to check that these snippets are well-formed. Other times we might want to process the code to obtain a result to also embed in the document.

Some may recognize this as similar to [literate programming](https://en.wikipedia.org/wiki/Literate_programming). The Emacs text editor has long has long shipped with a feature called [Org-mode](https://www.gnu.org/software/emacs/manual/html_node/emacs/Org-Mode.html) used for this kind of documentation. The Org-mode input files used by `org2gfm` are more-or-less a human-readable markdown files, though they support a lot of features for document processing.

To avoid having to bring up an entire text editor, just to generate documentation, Emacs can be run in a headless mode from the command-line. This is what `org2gfm` does. The script manages the execution of Emacs on the user's behalf.

Because `org2gfm` is provided by Nix, its instance of Emacs is safely isolated and hidden from any other instance of Emacs a user might have installed otherwise. This is helpful because instances of Emacs can be finicky to keep configured correctly.

We can also call `org2gfm` with an invocation of `nix shell` to make sure our documentation is executed in a controlled environment. This way, any commands referenced by code snippets can be guaranteed to be on `PATH`. This makes documentation generation even more repeatable, regardless of which machine we generate documentation on. New projects scaffolded with Nix-project are set up this way to call `org2gfm` with `nix shell`.
...
You can call `org2gfm` directly both with and without flakes. Using flakes, you can run `org2gfm` directly from GitHub with `nix shell` without even installing `org2gfm`:
...
We can use the `--ignore-environment` of `nix search` to sandbox execution of `org2gfm`. This clears out all environment variables. If we want to keep a few, we can use `nix search`'s `--keep` switch.

If we want more packages on the path than just `org2gfm` we can reference more flake packages. Here's an example:
...
Note when using `nix shell` with the `--file` we need all the packages for our sandbox to come from a single file (we can only use `--file` once). As a convenience, the Nix-project's `default.nix` file (accessed directly from a tarball retrieved from GitHub above) includes a snapshot of the latest stable release of Nixpkgs. We access this under `currentSystem.legacyPackages.nixpkgs`, as illustrated above.

If you need packages beyond what's provided in the Nixpkgs snapshot, you will need to create your own Nix expression in a file including everything you need for your `nix shell` execution. Explaining how is beyond the scope of this document.
...
The `org2gfm` script provided by this project constrains the full flexibility of [Emac's Org-mode](https://www.gnu.org/software/emacs/manual/html_node/emacs/Org-Mode.html), particularly with respect to evaluation and exporting. This section discusses the design of `org2gfm` and how to use it as intended.
...
## No evaluation when exporting<a id="sec-5-2"></a>
