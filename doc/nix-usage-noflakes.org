...
Using the =--file= option tells =nix repl= to load attributes from a Nix file.
...
aware that input parameters/options or output formatting might change with
later releases if you script using it.

We can use an =--extra-experimental-features nix-command= option to use an
...
in the current directory. If we omit the =--attr= option and argument,
...
=nix-build= has a =--no-out-link= option in case you want to build packages
...
Note, if you don't use the =--file= option, =nix shell= will assume you are
...
=--command= option and following arguments.
...
concise syntax than =nix shell= with a =--command= option. Like =nix search=,
...
the =--file= option. For example, here we reference a tarball of the
...
has changed, use the =--refresh= option with your invocation.

Since we can only specify one =--file= option, we can't make a shell with
...
options/arguments can still feel extraneous. It would be nice to have the
...
=--json= option to get more details:
...
can even roll back to an earlier profile with the =--rollback= option.  You can
...
program alive. Use =nix-collect-garbage='s =--delete-old= option to delete old
