...
** Installing and uninstalling programs
   ...
   ("{{{package-attr}}}") within the referenced Nix expression. But we
   uninstall it using the package name ("{{{package-name}}}"), which may or may
   not be the same as the attribute name. When a package is installed, Nix
   keeps no reference to the expression that evaluated to the derivation of the
