...
** Building Nix expressions
   ...
   =default.nix=.  This package provides the {{{package-type}}}
   {{{package-target}}}.
...
** Running commands
   ...
   take executables found in packages, put them in an environments =PATH=, and
   ...
   For example, to get the help message for the {{{run-target}}} provided by the
   {{{run-name}}} package selected by the {{{run-attr}}} attribute path from
   {{{run-src}}}, we can call the following:
   ...
   branch of "nixos-20.09" can be referenced by "channel:nixos-20.09" for =nix=
   subcommands that accept a =--file= switch.
