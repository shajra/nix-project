...
```
    ...
    /nix/store/mn9xh5sh6j0d1k0mrq9v00634ksd5qfb-nix-project
...
You can run a command from a package in a Nix expression with `nix run`. For instance, to get the help message for the `nix-project` executable provided by the "nix-project" package selected by the "nix-project-exe" attribute path, we can call the following:
