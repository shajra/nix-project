...
the [[file:project-developing-basics.org][Flakes Basic Development Guide]] or the [[file:project-developing-modules.org][Flake Module Development Guide]].
...
Because Nix provides =org2gfm= and all its dependencies, its instance of Emacs is
safely isolated and hidden from any other instance of Emacs a user might have
installed otherwise.  This isolation is helpful because instances of Emacs can
be finicky to keep configured correctly.

We can further use Nix to ensure  =org2gfm= is invoked in a controlled
environment. This way, any commands referenced by code snippets can be
guaranteed to be on =PATH=. This sandboxing makes documentation generation even
more repeatable, regardless of which machine we generate documentation on.
...
Nix-project (this project), see the [[file:project-developing-basics.org][Flakes Basic Development Guide]] and the [[file:project-developing-modules.org][Flake
...
** Calling the =org2gfm= executable directly

You can call =org2gfm= directly, either with or without flakes. Using flakes,
you can run =org2gfm= directly from GitHub with =nix run= without even
installing =org2gfm=:
...
If called with the =--evaluate= option, the code snippets in the Org file
are evaluated, changing the Org file in place.

If you don't want to evaluate all Org files found, files can be explicitly
specified as positional arguments, or the =--exclude= option can be used to
exclude filenames matching a regular expression.

Note the "-impure" name of the package indicates that all environment variables,
including =PATH= are available to the script during execution.  You could control
the environment with the =--ignore-environment= and =--keep-env-var= options of =nix
run=.  Alternatively, the next sections show how to generate a script with
locked-down environments using provided library support.

Use the =--help= option to see a complete list of options (or look at [[file:../nix/org2gfm.nix][the =org2gfm=
source code]]).

** Controlling the environment with the =org2gfm= library function

The Nix-project flake exports at =#lib.<system>.org2gfm= a function you
can use to wrap the =org2gfm= script with another script to tightly control the
environment =org2gfm= executes in, including the =PATH=.

See the [[file:../nix/org2gfm.nix][comments in the source code]] for details on how to call this function.

See the [[file:../examples/less][=less= template]] for an example of integrating this flake module into a
project. This template is discussed in the [[file:project-developing-basics.org][Flakes Basic Development Guide]].
...
first-hand via a alias set up in the developer environment called
=project-doc-gen=.

You can control the execution of =org2gfm= by modifying [[file:../examples/less/config.nix][=config.nix=]] of the =less=
template.

** Using the =org2gfm= flake module

The Nix-project flake exports a flake module at =#flakeModules.org2gfm= that
allows you to configure your project using [[flake-parts][=flake-parts=]] to compile a version of
=org2gfm= tailored to your needs.

See the [[file:../nix/module/org2gfm.nix][flake module source code]] for details on the options this module accepts.

See the [[file:../examples/more][=more= template]] for an example of integrating this flake module into
a project. This template is discussed in the [[file:project-developing-modules.org][Flake Module Development Guide]].
...
If you call =org2gfm= with the =--evaluate= option, all sources will be evaluated
unless the block is explicitly set ~:eval no~. Without this option, =org2gfm= does
not evaluate at all.
