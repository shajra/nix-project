...
extremely portable.  If a computer has Nix installed, our projects should be
able to download and build all the dependencies needed.  For the most part this
makes the installation of Nix the only required dependency for our projects.
Beyond Nix, we can spare bothering asking users to install and configure
complicated tool chains.  Nix's infrastructure supports a huge variety of
language ecosystems.
