...
* About this project
  ...
  This project helps set up projects use the [[nix][Nix package manager]] better.
  Specifically, it provides two scripts:

  - =nix-project= to help scaffold a Nix-based project and update dependencies with [[niv][Niv]].
  - =org2gfm= to generate [[gfm][GitHub Flavored Markdown (GFM)]] from [[org][Emacs Org-mode]] files.
...
** Motivation to use Nix
   ...
   guaranteed not to conflict with anything you already have installed in a
   system. Nix pulls down dependencies that aren't on a system. And Nix can
   ...
   where native tooling stops, layering on top of the tools and techniques we're
...
** Managing dependencies with Nix and Niv
   ...
   explicitly provided hash. Each new dependency (or new version of a
   dependency) requires the specification of a new hash. But these hashes can be
   a chore to maintain when upgrading many dependencies.
   ...
   pre-installed. Although not often a problem, Niv's reliance on preexisting
   installations isn't in the spirit of Nix. We want Nix to download all
   dependencies for us. That's why we're using Nix in the first place.
...
** Documenting with Emacs Org-mode
   ...
   results in blocks right below the code. And this can then be exported to the
   format of our choice.

   Manually managing an Emacs installation, including requisite plug-ins, is
   ...
   execution of Emacs to generate our documentation. Note that as its name
   implies =org2gfm= only generates [[gfm][GitHub Flavored Markdown (GFM)]] from
   Org-mode files. This is currently all the author needs for projects already
   hosted on GitHub.
...
** Scaffolding
   ...
   provides some packages, and we're selecting the =nix-project= one. The
   ...
   ~PATH~. The =nix-project= package provides a =nix-project= script, which we
   call with the ~--scaffold~ argument for scaffolding our empty directory. And
   finally, we use the ~--nix~ switch to indicate which =nix= executable to
...
** Managing dependencies
   ...
   commands directly with =dependencies-upgrade=. For instance, to run the
   ...
   You can also use the ~--help~ switch with both =dependencies-upgrade= and
   =niv=:
...
* License
  ...
  All files in this "org2gfm" project are licensed under the terms of
...
* Contribution
  ...
  Feel free to file issues and submit pull requests with GitHub.  For now, this
  script meets a very specific documentation need and doesn't have too much.
