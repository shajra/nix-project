...
** Managing dependencies with Nix and Niv
   ...
   upgrading dependencies a single command. Niv tracks downloaded versions of
   source from locations like GitHub, maintaining metadata of the latest
   versions and calculated hashes in a =sources.json= file. Since this project
   uses itself, you can have a look at [[file:nix/sources.json][this project's =sources.json= file]] as an
   example.
...
** Documenting with Emacs Org-mode
   ...
   [[org][Emacs' Org-mode]] has some compelling features in the spirit of literate
   programming. With Org-mode, we can evaluate code blocks and inject the
   results inline right below the code. And this all can then be exported to
...
** Next steps
   ...
   If you haven't looked it yet, this project provides [[file:doc/nix.org][some documentation on
   Nix]] to get you started.
